...
import Student from '@src/components/Solicitation/TeacherSolicitation/Columns/Student';
...
function TeacherSolicitations ({ isCoordinator }) {
  const dispatch = useDispatch();
  //Redux state
  const user = useSelector((state) => state.authentication.user);
  const studentSolicitations = useSelector((state) => state.solicitations.teacherSolicitations);
  ...
  const coordOrTeacherFilters = useMemo(
    () => ({
      coordinator: isCoordinator ? get(user, 'person._id') : undefined,
      teacher: isCoordinator ? undefined : get(user, 'person._id')
    }),
    [isCoordinator, user]
  );
  ...
  const renderUpdateModal = () => {
    return (
      ...
      >
        <UpdateForm
          coordinator={isCoordinator ? user : undefined}
          teacher={isCoordinator ? undefined : user}
  ...
  return (
    <div className="container_home">
      <ComponentHeader title={`${ isCoordinator ? 'Coordinator' : 'Teacher' } Solicitations`} />
...
export default TeacherSolicitations;
